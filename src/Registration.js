...
const App = () => {
  ...
  const { name, email, password, confirmPassword, gender,user } = values
  ...
  const submitHandler = (e) => {
    e.preventDefault()
     axios.post('https://project-18f20050-9fa9-4ad5-b7d-default-rtdb.firebaseio.com/regis.json',values)
    if (!name) {
    ...
    if (!user) {
  alert("Please select User/Admin");
  return;
    ...
    const newUser = { id: Date.now(), name, email, password, gender,user };
  ...
  return (
    <center>
      ...
      <form onSubmit={submitHandler}>
        ...
        <br />
        <br />
        <select name="user" value={user}  onChange={changeHandler}>
        ...
        <button type="button" onClick={() => navigate("/loginform")}>Login</button>
      </form>
    </center>
  )
}
